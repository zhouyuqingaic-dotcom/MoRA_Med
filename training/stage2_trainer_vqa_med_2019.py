...
# 1. 导入 VQA_MED 2019 专属配置类
from config.vqa_med_2019.stage2_train_config_vqa_med_2019 import (
    Stage2TrainConfig,
)
# 2. 导入 VQA_MED 2019 数据集
from datas.vqa_med_2019_datasets import VQAMED2019Dataset
# 3. 导入 VQA_MED 2019 专属 Collator
from utils.data_tools.collator.vqa_med_2019.vqa_med_2019_train_collator import (
    VQAMED2019TrainCollator,
)
...
from utils.data_tools.prompt_cleaning.vqa_med_2019_answer_cleaning import (
    vqa_med_2019_answer_train_cleaning,
)

def filter_invalid_vqa_med_2019_samples(
    dataset,
    split_name: str,
    print_rank: int,
):
    ...
    for index in range(original_count):
        ...
        answer_text = vqa_med_2019_answer_train_cleaning(
            answer_source
        )
        ...
        if question_text and answer_text and image_exists:
    ...
    ddp_print(
        f"[VQA-Med 2019 数据清理] {split_name}: "
        f"原始={original_count}, "
...
def main():
    ...
    ddp_print(
        f"🚀 [1/6] 启动 RoMA-Net V2-lite Stage 2 VQA_Med 2019 训练！"
    ...
    # ==========================================
    # 1. 加载并合并 VQA-Med 2019 Train + Validation
    # ==========================================
    ddp_print(
        "⏳ [2/6] 正在加载并合并 "
        "VQA-Med 2019 Train 和 Validation 数据集...",
        print_rank=cfg.print_rank,
    )
    ...
    train_subset = filter_invalid_vqa_med_2019_samples(
        dataset=train_subset,
        split_name="train",
        print_rank=cfg.print_rank,
    )

    val_subset = filter_invalid_vqa_med_2019_samples(
        dataset=val_subset,
    ...
    # ==========================================
    # 5. 挂载 VQA-Med 2019 专属 Collator
    # ==========================================
    ddp_print(
        "\n⏳ [5/6] 挂载 VQA-Med 2019 专属 Collator...",
        print_rank=cfg.print_rank,
    )
    ...
    # ==========================================
    # 7. 保存最终权重 (Stage 2 VQA_Med 2019 结业)
    # ==========================================
    if local_rank in [-1, 0]:
        ...
        ddp_print(f"\n🎉 Stage 2 (VQA_Med 2019) 训练完成！LoRA 已保存至: {final_save_path}", print_rank=cfg.print_rank)
