...
from torch.utils.data import Subset  # 🚀 降维打击核心工具：缝合数据集
...
def filter_invalid_vqa_rad_samples(
    dataset,
    split_name: str,
    print_rank: int,
):
    """
    过滤缺少有效问题、答案或图像的 VQA-RAD 样本。
    """
    ...
    if len(filtered_dataset) == 0:
        raise RuntimeError(
            f"VQA-RAD {split_name} 过滤后没有有效样本。"
        )
...
def main():
    ...
    ddp_print(
        f"🚀 [1/6] 启动 MoRA-Med Stage 2 VQA-RAD 训练！"
    ...
    ddp_print(
        ...
        f"use_discriminative_lr="
        f"{getattr(cfg, 'use_discriminative_lr', False)}",
        print_rank=cfg.print_rank,
    )
    ...
    train_dataset = filter_invalid_vqa_rad_samples(
    ...
    if max_train_samples is not None:
        ...
        if max_train_samples <= 0:
            raise ValueError(
                "max_vqa_rad_train_samples 必须大于 0，"
                "或者设置为 None。"
            )
        ...
        ddp_print(
            f"ℹ️ 已启用 VQA-RAD 样本截断："
            f"{sample_count} 条。",
            print_rank=cfg.print_rank,
        )

    ddp_print(
        f"✅ VQA-RAD 训练集加载完成，共有 "
        f"{len(train_dataset)} 条有效训练样本。",
        print_rank=cfg.print_rank,
    )
    ...
    # ==========================================
    # 5. 🚀 挂载 VQA_RAD 专属 Collator
    # ==========================================
    ddp_print(
        "\n⏳ [5/6] 挂载 VQA-RAD 专属 Collator...",
        print_rank=cfg.print_rank,
    )
    ...
    if local_rank in [-1, 0]:
        ...
        ddp_print(
            f"\n🎉 Stage 2 (VQA-RAD) 训练完成！"
            f"LoRA 已保存至：{final_save_path}",
            print_rank=cfg.print_rank,
        )
