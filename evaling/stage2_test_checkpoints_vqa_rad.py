    ...
    with torch.no_grad():
        for batch_inputs, metadata in tqdm(
            ...
            for i, prediction in enumerate(
                ...
                # VQA_RAD 中 CLOSED 通常对应 yes/no 等封闭式问题；
                # 其余归为 OPEN。
                category = (
                    "closed"
                    if (
...
def main():
    ...
    # =========================================================
    # 3. VQA-RAD Test DataLoader
    # =========================================================
    dataset = VQARADDataset(
        jsonl_path=(
            cfg.vqa_rad_test_jsonl_path
        ),
        image_root=(
            cfg.vqa_rad_image_root
        ),
    )

    collator = VQARADEvalCollator(
        processor=processor,
        cfg=cfg,
        biomed_transform=(
            biomed_transform
